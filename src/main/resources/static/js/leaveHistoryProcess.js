$(document).ready(function () {
    ...
    var grid = $("#grid-data").bootgrid({
        ...
        ajax: true,
        url: "getfinishprocess?type=leave",
        formatters: {
            "process_instance_id": function (column, row) {
                return row.data.process_instance_id;
            },
            "user_id": function (column, row) {
                return row.data.user_id;
            },
            "leave_type": function (column, row) {
                return row.data.leave_type;
            },
            "apply_time": function (column, row) {
                return row.data.apply_time;
            },
            "commands": function (column, row) {
                return "<button class=\"btn btn-xs btn-default ajax-link command-run1\" data-row-id=\"" + row.data.process_instance_id + "\">查看详情</button>";
            }
        }
